import { Link } from "react-router-dom";
import Faq from "../../Shared/Faq";
import PhaseOfdevelopments from "../../Shared/PhaseOfdevelopments";

function Whatsappchatbot() {
  return (
    <>
      <section className="wrapper !bg-[#eff7fa]">
        <div className="container pt-5 pb-20 lg:py-28 xl:py-36 xl:!pb-[10rem] !relative">
          <img
            className="xl:!absolute lg:!absolute max-lg:!left-0 w-full lg:w-10/12 xl:w-11/12 xxl:w-10/12 flex-[0_0_auto] max-w-full lg:!px-5 xl:!px-0 !ml-[-1.25rem] sm:!ml-[-2rem] md:!ml-[-2.5rem] lg:!ml-0 xl:!ml-0 md:!mb-4 lg:!mb-0"
            src="https://sandbox-tailwind-template.netlify.app/assets/img/photos/devices.png"
            srcSet="https://sandbox-tailwind-template.netlify.app/assets/img/photos/devices.png"
            alt="image"
            style={{ top: "-1%", left: "-25%" }}
          />
          <div className="flex flex-wrap mx-0 items-center">
            <div className="md:w-10/12 lg:w-5/12 xl:w-5/12 flex-[0_0_auto] max-w-full md:!ml-[8.33333333%] lg:!ml-[58.33333333%] xxl:!ml-[50%] xxl:!pl-14 xl:!mt-[-2.25rem] lg:!mt-[-2.25rem] md:!mt-[-2.25rem] text-center lg:text-left xl:text-left">
              <div className="flex items-center space-x-2 mb-4">
                <img
                  src="https://www.spcdn.org/templates/sendpulsev1/img/svg-icons/ic-official-gray.svg"
                  alt="Official Icon"
                  className="w-5 h-5"
                />
                <span className="text-sm font-medium text-gray-600">
                  Official WhatsApp Business Solution Provider
                </span>
              </div>

              <h1 className="xl:!text-[2.3rem] !text-[calc(1.355rem_+_1.26vw)] !leading-[1.2] font-bold !text-[#343f52] !tracking-[-0.01rem] !mb-4 sm:mx-[-0.5rem] md:mx-0">
                WhatsApp сhatbots
              </h1>
              <p className="lead !text-[1.1rem] !leading-[1.55] font-medium !mb-7 md:!px-10 lg:!px-0">
                All user-initiated conversations are free. No setup or extra
                fees for the WhatsApp Business API
              </p>
              <div className="flex justify-center lg:!justify-start xl:!justify-start">
                <span>
                  <Link
                    to="#"
                    className="!text-[.85rem] btn btn-aqua !text-white !bg-[#54a8c7] border-[#54a8c7] hover:text-white hover:bg-[#54a8c7] hover:!border-[#54a8c7] focus:shadow-[rgba(79,152,181,1)] focus:text-white active:text-white active:bg-[#54a8c7] active:border-[#54a8c7] disabled:text-white disabled:bg-[#54a8c7] disabled:border-[#54a8c7] btn-icon btn-icon-start rounded !mr-2"
                  >
                    Craete a Chatboat
                  </Link>
                </span>
                <span>
                  <Link
                    to="/contact"
                    className="!text-[.85rem] btn btn-green !text-white !bg-[#45c4a0] border-[#45c4a0] hover:text-white hover:bg-[#45c4a0] hover:!border-[#45c4a0]   active:text-white active:bg-[#45c4a0] active:border-[#45c4a0] disabled:text-white disabled:bg-[#45c4a0] disabled:border-[#45c4a0]  btn-icon btn-icon-start rounded"
                  >
                    Conatct Us
                  </Link>
                </span>
              </div>
            </div>
          </div>
        </div>
      </section>
      <section className="wrapper !bg-[#ffffff]">
        <div className="container py-[4.5rem] xl:!py-[7rem] lg:!py-[7rem] md:!py-[7rem]">
          <div className="flex flex-wrap mx-[-15px] !text-center xl:!mt-[3.5rem]">
            <div className="md:w-10/12 xl:w-8/12 lg:w-8/12 w-full flex-[0_0_auto] !px-[15px] max-w-full xl:!ml-[16.66666667%] lg:!ml-[16.66666667%] md:!ml-[8.33333333%]">
              <h2 className="!text-[.75rem] uppercase !text-[#aab0bc] !mb-3 !tracking-[0.02rem] !leading-[1.35]">
                Start using WhatsApp chatbots for your business
              </h2>
              <h3 className="xl:!text-[1.9rem] !text-[calc(1.315rem_+_0.78vw)] !leading-[1.25] font-bold !mb-9 xl:!px-12">
                Automate conversations with your customers, from the first touch
                point to closing the sale
              </h3>
            </div>
          </div>
          <div className="flex flex-wrap mx-[-15px] xl:mx-[-35px] lg:mx-[-20px] !mt-[-40px] !mb-[4.5rem] xl:!mb-[7rem] lg:!mb-[7rem] md:!mb-[7rem]">
            <div className="md:w-6/12 lg:w-4/12 xl:w-3/12 w-full flex-[0_0_auto] xl:!px-[35px] lg:!px-[20px] !px-[15px] max-w-full !mt-[40px]">
              <div className="flex flex-row">
                <div>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 512 512"
                    className="svg-inject icon-svg !w-[2.2rem] !h-[2.2rem]  !text-[#54a8c7] text-aqua !mr-4"
                  >
                    <circle
                      className="lineal-fill"
                      cx="283.8"
                      cy="283.8"
                      r="143"
                    ></circle>
                    <path
                      className="lineal-stroke"
                      d="M283.8 441.8c-87.1 0-158-70.9-158-158s70.9-158 158-158 158 70.9 158 158-70.9 158-158 158zm0-286.1c-70.7 0-128.1 57.3-128.1 128.1S213 411.9 283.8 411.9s128.1-57.3 128.1-128.1c-.1-70.7-57.4-128-128.1-128.1z"
                    ></path>
                    <path
                      className="lineal-fill"
                      d="M283.8 210.5c40.5 0 73.3 32.8 73.3 73.3s-32.8 73.3-73.3 73.3-73.3-32.8-73.3-73.3"
                    ></path>
                    <path
                      className="lineal-stroke"
                      d="M283.8 372.1c-48.7-.1-88.2-39.5-88.2-88.3.2-8.2 7.1-14.8 15.3-14.5 7.9.2 14.3 6.6 14.5 14.5 0 32.2 26.1 58.4 58.4 58.4s58.4-26.1 58.4-58.4-26.1-58.4-58.4-58.4c-8.2-.2-14.8-7.1-14.5-15.3.2-7.9 6.6-14.3 14.5-14.5 48.7-.5 88.7 38.6 89.2 87.3s-38.6 88.7-87.3 89.2h-1.9z"
                    ></path>
                    <path
                      className="lineal-fill"
                      d="M107.9 55.1l-5.3 47.5-47.5 5.3-40.2-40.1 52.9-52.9z"
                    ></path>
                    <path
                      className="lineal-stroke"
                      d="M283.8 55.7c-55.3 0-108.8 20.1-150.4 56.7l-15.2-15.2 4.5-40.4c.5-4.5-1.1-9-4.3-12.2L78.3 4.4c-5.8-5.8-15.3-5.8-21.1 0L4.4 57.2c-5.8 5.8-5.8 15.3 0 21.1l40.1 40.1c2.8 2.8 6.6 4.4 10.6 4.4.5 0 1.1 0 1.6-.1l40.4-4.5 15.2 15.2c-36.6 41.5-56.7 95-56.7 150.4C55.7 409.6 158 512 283.8 512c66.2 0 129.1-28.7 172.4-78.7 5.6-6.1 5.1-15.6-1-21.1s-15.6-5.1-21.1 1l-.5.5c-37.7 43.4-92.3 68.4-149.8 68.4-109.3 0-198.3-89-198.3-198.3 0-47.4 17-93.3 48-129.2l140.2 140.2c5.8 5.8 15.3 5.8 21.1 0s5.8-15.3 0-21.1L154.6 133.5c35.9-31 81.8-48 129.2-48 109.4 0 198.3 89 198.3 198.3 0 8.3 6.7 14.9 14.9 14.9s14.9-6.7 14.9-14.9C512 158 409.6 55.7 283.8 55.7zM36.1 67.8l31.7-31.7 24.5 24.5-3.2 28.5-28.5 3.2-24.5-24.5z"
                    ></path>
                  </svg>
                </div>
                <div>
                  <h4 className="!mb-1 !text-[1rem] !leading-[1.45]">
                    Increase user engagement
                  </h4>
                  <p className="!mb-0">
                    Subscribers are more likely to open triggered messages from
                    your bot than bulk campaigns.
                  </p>
                </div>
              </div>
            </div>

            <div className="md:w-6/12 lg:w-4/12 xl:w-3/12 w-full flex-[0_0_auto] xl:!px-[35px] lg:!px-[20px] !px-[15px] max-w-full !mt-[40px]">
              <div className="flex flex-row">
                <div>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 316.5 409.6"
                    className="svg-inject icon-svg !w-[2.2rem] !h-[2.2rem]  !text-[#fab758] text-yellow !mr-4"
                  >
                    <path
                      className="lineal-stroke"
                      d="M158.2 409.6c-59.4 0-107.5-48.2-107.5-107.5S99 194.6 158.3 194.6s107.5 48.2 107.5 107.5c0 20-5.6 39.6-16.1 56.6-3.4 5.5-10.6 7.1-16 3.7s-7.1-10.6-3.7-16c24.4-39.5 12.2-91.3-27.3-115.8S111.3 218.5 86.8 258s-12.2 91.3 27.3 115.8c22.5 13.9 50.2 16.4 74.8 6.8 6-2.3 12.7.6 15.1 6.6 2.3 6-.6 12.7-6.6 15.1-12.5 4.8-25.8 7.3-39.2 7.3z"
                    ></path>
                    <path
                      className="lineal-fill"
                      d="M158.2 206.2c19.9 0 39.4 6.2 55.6 17.8l88.1-152.5c4.6-8 3.7-18-2.2-25l-23-27.1c-4.1-4.9-10.2-7.7-16.6-7.7H56.4c-6.4 0-12.5 2.8-16.6 7.7l-23 27.2c-6 7-6.9 17-2.2 25l88 152.4c16.2-11.6 35.7-17.8 55.6-17.8zm73.4-134l-73.3 127-73.4-127h146.7z"
                    ></path>
                    <path
                      className="lineal-stroke"
                      d="M213.8 235.6c-2.4 0-4.8-.8-6.8-2.2-29.2-20.8-68.5-20.8-97.7 0-5.2 3.7-12.5 2.5-16.2-2.7l-.6-.9-88-152.5c-7-12.2-5.7-27.5 3.4-38.3l23-27.2C37.2 4.3 46.6 0 56.4 0h203.7c9.8 0 19.2 4.3 25.5 11.8l23 27.1c9.1 10.8 10.5 26.1 3.4 38.3l-88.1 152.5c-1.6 2.8-4.4 4.9-7.6 5.6-.8.2-1.6.3-2.5.3zM56.4 23.3c-3 0-5.8 1.3-7.8 3.6L25.7 54c-2.8 3.3-3.2 7.9-1 11.7l82.1 142c11.2-6.1 23.4-10.2 36-12.1L74.8 78c-3.2-5.6-1.3-12.7 4.3-15.9 1.8-1 3.8-1.6 5.8-1.6h146.7c6.4 0 11.6 5.2 11.6 11.7 0 2-.5 4-1.6 5.8l-67.9 117.7c12.6 1.8 24.8 5.9 36 12.1l82-142c2.1-3.7 1.7-8.4-1-11.6l-23-27.2c-1.9-2.3-4.8-3.6-7.7-3.6l-203.6-.1zm48.7 60.5l53.2 92.1 53.2-92.1H105.1zm45.1 192.9l-4.4 2.7c-1.1.8-2.4 1.2-3.8 1.2-4.4 0-7.4-4.7-7.4-9.4-.1-3.3 1.6-6.3 4.4-8l20-12.1c1.3-.8 2.9-1.1 4.4-1.1 4.8 0 10.4 2.9 10.4 7.4v96.2c0 4.9-5.9 7.3-11.8 7.3s-11.8-2.4-11.8-7.3v-76.9z"
                    ></path>
                  </svg>
                </div>
                <div>
                  <h4 className="!mb-1 !text-[1rem] !leading-[1.45]">
                    Convert leads with ease
                  </h4>
                  <p className="!mb-0">
                    Convert leads into customers by sending valuable content in
                    your automated flows.
                  </p>
                </div>
              </div>
            </div>

            <div className="md:w-6/12 lg:w-4/12 xl:w-3/12 w-full flex-[0_0_auto] xl:!px-[35px] lg:!px-[20px] !px-[15px] max-w-full !mt-[40px]">
              <div className="flex flex-row">
                <div>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 409.6 391.2"
                    className="svg-inject icon-svg !w-[2.2rem] !h-[2.2rem]  !text-[#e2626b] text-red !mr-4"
                  >
                    <path
                      className="lineal-fill"
                      d="M148.9 61.3C136 24.4 95.7 4.9 58.8 17.8S2.4 71.1 15.3 108c6.5 18.5 20.3 33.5 38.3 41.4 15.9-42.8 51.3-75.5 95.3-88.1z"
                    ></path>
                    <path
                      className="lineal-stroke"
                      d="M53.5 160.7c-1.6 0-3.1-.4-4.6-1C7.5 141.4-11.3 93 7 51.5s66.8-60.3 108.2-42c20.8 9.2 36.9 26.6 44.4 48.1 2.1 5.9-1 12.3-6.9 14.4-.2.1-.4.1-.6.2-40.5 11.6-73.1 41.7-87.9 81.1-1.8 4.5-6 7.4-10.7 7.4zM82.1 25.2c-32.8 0-59.5 26.6-59.5 59.5 0 19.4 9.5 37.7 25.4 48.8C65.2 97.2 95.8 69 133.4 54.7c-10.7-18.3-30.2-29.5-51.3-29.5z"
                    ></path>
                    <path
                      className="lineal-fill"
                      d="M327.5 13.9c-30.1 0-56.9 19-66.8 47.5 44 12.6 79.4 45.2 95.4 88.1 35.8-15.8 52-57.6 36.2-93.3-11.3-25.8-36.7-42.3-64.8-42.3z"
                    ></path>
                    <path
                      className="lineal-stroke"
                      d="M356.1 160.7c-4.7 0-8.9-2.9-10.6-7.4-14.8-39.5-47.4-69.6-87.9-81.2-6-1.7-9.5-8-7.8-14 .1-.2.1-.4.2-.6C264.9 14.7 311.7-7.9 354.5 7S420 68.8 405 111.7c-7.5 21.5-23.6 38.9-44.4 48.1-1.4.6-2.9.9-4.5.9zM276.2 54.6c37.6 14.3 68.1 42.5 85.4 78.8 26.9-18.8 33.5-55.9 14.7-82.8s-55.9-33.5-82.8-14.7c-7.1 4.9-13 11.3-17.3 18.7zm-79.4 176c-3.1 0-6.1-.7-8.8-2.1l-45.1-22.6c-5.6-2.8-7.8-9.6-5-15.2s9.6-7.8 15.2-5l43.3 21.6 57.4-57.4c4.4-4.4 11.6-4.4 16 0s4.4 11.6 0 16l-58.9 58.9c-3.8 3.7-8.8 5.8-14.1 5.8z"
                    ></path>
                    <path
                      className="lineal-stroke"
                      d="M204.8 391.2c-96.8 0-175.6-78.8-175.6-175.6S108 39.9 204.8 39.9c69.8 0 133 41.3 161 105.2 2.5 5.7-.1 12.4-5.9 14.9s-12.4-.1-14.9-5.9c-24.4-55.7-79.4-91.7-140.2-91.7-84.4 0-153 68.6-153 153s68.6 153 153 153 153-68.6 153-153c-.2-6.2 4.7-11.5 11-11.6 6.2-.2 11.5 4.7 11.6 11v.7c0 96.9-78.8 175.7-175.6 175.7z"
                    ></path>
                    <path
                      className="lineal-stroke"
                      d="M287.1 281.5c-1.9 0-3.7-.5-5.4-1.4l-102.4-55.7c-5.5-3-7.5-9.8-4.5-15.3s9.8-7.5 15.3-4.5l102.5 55.7c5.5 3 7.5 9.8 4.5 15.3-2 3.6-5.8 5.9-10 5.9zm-82.3-178.4c-6.2 0-11.3-5-11.3-11.3V51.2c0-6.2 5.1-11.3 11.3-11.3s11.3 5.1 11.3 11.3v40.5c0 6.3-5 11.3-11.3 11.4zm0 288.1c-6.2 0-11.3-5.1-11.3-11.3v-40.5c0-6.2 5.1-11.3 11.3-11.3s11.3 5.1 11.3 11.3v40.5c0 6.2-5 11.3-11.3 11.3zm164.3-164.3h-40.5c-6.2.2-11.5-4.7-11.6-11-.2-6.2 4.7-11.5 11-11.6h41.2c6.2.2 11.2 5.4 11 11.6-.3 6-5.1 10.8-11.1 11zm-288.1 0H40.5c-6.2-.2-11.2-5.4-11-11.6.2-6 5-10.8 11-11H81c6.2.2 11.2 5.4 11 11.6-.2 6-5 10.8-11 11z"
                    ></path>
                    <path
                      className="lineal-stroke"
                      d="M241.2 62.5c-6.2 0-11.3-5.1-11.3-11.3V27.9c0-2.9-2.4-5.3-5.3-5.3H185c-2.9 0-5.3 2.4-5.3 5.3v23.4c0 6.2-5.1 11.3-11.3 11.3s-11.3-5.1-11.3-11.3V27.9C157.1 12.5 169.6 0 185 0h39.6c15.4 0 27.9 12.5 27.9 27.9v23.4c-.1 6.2-5.1 11.2-11.3 11.2z"
                    ></path>
                  </svg>
                </div>
                <div>
                  <h4 className="!mb-1 !text-[1rem] !leading-[1.45]">
                    Get customer feedback
                  </h4>
                  <p className="!mb-0">
                    Build a better product or service by getting reviews from
                    your customers.
                  </p>
                </div>
              </div>
            </div>

            <div className="md:w-6/12 lg:w-4/12 xl:w-3/12 w-full flex-[0_0_auto] xl:!px-[35px] lg:!px-[20px] !px-[15px] max-w-full !mt-[40px]">
              <div className="flex flex-row">
                <div>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 409.6 409.6"
                    className="svg-inject icon-svg !w-[2.2rem] !h-[2.2rem]  !text-[#d16b86] text-pink !mr-4"
                  >
                    <path
                      className="lineal-stroke"
                      d="M204.8 409.6C91.9 409.6 0 317.7 0 204.8c0-49.9 18.2-98.1 51.2-135.5 4.4-5 12-5.5 17-1.1s5.5 12 1.1 17c-29.1 33-45.2 75.5-45.2 119.5 0 99.6 81.1 180.7 180.7 180.7s180.7-81.1 180.7-180.7S304.4 24.1 204.8 24.1c-27.3-.1-54.2 6.1-78.7 18-6 2.9-13.2.4-16.1-5.6-2.9-6-.4-13.2 5.6-16.1C143.4 6.9 173.9-.1 204.8 0c112.9 0 204.8 91.9 204.8 204.8s-91.9 204.8-204.8 204.8z"
                    ></path>
                    <path
                      className="lineal-fill"
                      d="M349.4 204.8c0 79.8-64.7 144.6-144.6 144.6S60.2 284.6 60.2 204.8 125 60.2 204.8 60.2 349.4 125 349.4 204.8z"
                    ></path>
                    <path
                      className="lineal-stroke"
                      d="M204.8 361.4c-86.4 0-156.6-70.2-156.6-156.6S118.4 48.2 204.8 48.2s156.6 70.2 156.6 156.6-70.2 156.6-156.6 156.6zm0-289.1c-73.1 0-132.5 59.4-132.5 132.5s59.4 132.5 132.5 132.5 132.5-59.5 132.5-132.5S277.9 72.3 204.8 72.3z"
                    ></path>
                    <path
                      className="lineal-stroke"
                      d="M200.9 246.7c-8.8 0-17.2-3.5-23.5-9.7L145 204.5c-4.7-4.7-4.7-12.3 0-17s12.3-4.7 17 0l32.5 32.5c3.6 3.5 9.3 3.5 12.8 0l49.8-49.9c4.7-4.7 12.3-4.7 17 0s4.7 12.3 0 17L224.4 237c-6.2 6.2-14.7 9.7-23.5 9.7z"
                    ></path>
                  </svg>
                </div>
                <div>
                  <h4 className="!mb-1 !text-[1rem] !leading-[1.45]">
                    Provide 24/7 support
                  </h4>
                  <p className="!mb-0">
                    Create live chats to provide support and speedy answers
                    24/7.
                  </p>
                </div>
              </div>
            </div>
          </div>
          <div className="flex flex-wrap mx-[-15px] xl:mx-[-25px] lg:mx-[-20px] items-center">
            <div className="xl:w-6/12 lg:w-6/12 w-full flex-[0_0_auto] xl:!px-[25px] lg:!px-[20px] !px-[15px] max-w-full">
              <figure className="m-0 p-0">
                <img
                  className="w-full max-w-full !h-auto"
                  src="https://www.spcdn.org/images/whatsapp/whatsapp-en/pic_short-meaningful-pop01-en.png"
                  srcSet="https://www.spcdn.org/images/whatsapp/whatsapp-en/pic_short-meaningful-pop01-en.png"
                  alt="image"
                />
              </figure>
            </div>

            <div className="xl:w-6/12 lg:w-6/12 w-full flex-[0_0_auto] xl:!px-[25px] lg:!px-[20px] !px-[15px] max-w-full">
              <h3 className="!text-[calc(1.315rem_+_0.78vw)] font-bold xl:!text-[1.8rem] !leading-[1.25] !mb-5">
                Grow your WhatsApp audience
              </h3>
              <p className="!mb-8">
                Attract more chatbot subscribers by leveraging a website
                subscription widget, an automatically generated bio link page,
                or a QR code linked to your welcome flow. You can also send
                email or SMS invitations to your existing audiences, encouraging
                them to subscribe to your WhatsApp chatbot.
              </p>
              <div className="flex flex-wrap mx-[-15px] !mt-[-30px] xxl:mx-[-20px] process-wrapper">
                <div className="xl:w-4/12 lg:w-4/12 md:w-4/12 w-full flex-[0_0_auto] xxl:!px-[20px] !px-[15px] max-w-full !mt-[30px]">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 409.6 319.1"
                    className="svg-inject icon-svg !w-[2.2rem] !h-[2.2rem]  !text-[#45c4a0] text-green !mb-3"
                  >
                    <path
                      className="lineal-stroke"
                      d="M333.9 278.3h-47c-6-.2-10.8-5.2-10.6-11.2.2-5.8 4.8-10.4 10.6-10.6h47c29.8 0 53.9-24.1 53.9-53.9 0-25.9-18.5-48.1-43.9-52.9-5.9-1.1-9.8-6.8-8.7-12.7 0-.2.1-.5.2-.7 12.7-48.9-16.6-98.8-65.5-111.4-38.4-10-78.9 6-100.2 39.5-2.9 4.6-8.8 6.3-13.7 4.1-30.1-13.6-65.6-.2-79.2 29.9-3.5 7.8-5.3 16.2-5.3 24.7 0 4.6.6 9.3 1.6 13.8 1.4 5.9-2.3 11.7-8.1 13.1h-.2c-29.3 6-47.9 34.5-41.8 63.7 5.2 24.9 27.2 42.8 52.7 42.8H123c6-.2 11.1 4.6 11.2 10.6.2 6-4.6 11.1-10.6 11.2H75.7C33.9 278.3 0 244.5 0 202.7c0-31.9 20-60.4 50-71.3-.3-2.8-.4-5.6-.5-8.5 0-45.1 36.6-81.7 81.7-81.7 8.5 0 17 1.3 25.1 3.9 37.6-50 108.6-60 158.5-22.4C348.4 48 365.2 89.6 358.6 131c39.5 13.6 60.5 56.7 46.9 96.2-10.6 30.6-39.3 51.1-71.6 51.1z"
                    ></path>
                    <path
                      className="lineal-fill"
                      d="M159 251.2l39.2 53.6c2.7 3.6 7.8 4.4 11.4 1.8.7-.5 1.3-1.1 1.8-1.8l39.3-53.6c2.7-3.6 1.9-8.7-1.7-11.4-1.4-1-3.1-1.6-4.9-1.6h-10.7V186c0-10.3-8.3-18.6-18.6-18.6H195c-10.3 0-18.6 8.4-18.6 18.6v52.2h-10.7c-4.5 0-8.2 3.6-8.2 8.1-.1 1.8.4 3.5 1.5 4.9z"
                    ></path>
                    <path
                      className="lineal-stroke"
                      d="M204.8 319.1c-6.1 0-11.8-2.9-15.4-7.8l-39.3-53.6c-6.2-8.5-4.4-20.4 4.1-26.7 3.2-2.3 7.1-3.6 11.1-3.7V186c0-16.3 13.2-29.5 29.5-29.6h19.8c16.3 0 29.5 13.2 29.6 29.6v41.3c10.5.1 19 8.8 18.8 19.3 0 4-1.3 7.8-3.7 11L220 311.2c-3.4 5-9.1 7.8-15.2 7.9zm-33.8-70l33.8 46.2 33.8-46.2h-5.3c-6 0-10.9-4.9-10.9-10.9V186c0-4.3-3.5-7.7-7.7-7.7h-19.9c-4.3 0-7.7 3.5-7.7 7.7v52.2c0 6-4.9 10.9-10.9 10.9H171z"
                    ></path>
                  </svg>
                  <h4 className="!mb-1">1. Website Subscription Widget</h4>
                </div>

                <div className="xl:w-4/12 lg:w-4/12 md:w-4/12 w-full flex-[0_0_auto] xxl:!px-[20px] !px-[15px] max-w-full !mt-[30px]">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 306.7 409.6"
                    className="svg-inject icon-svg !w-[2.2rem] !h-[2.2rem]  !text-[#e2626b] text-red !mb-3"
                  >
                    <path
                      className="lineal-stroke"
                      d="M11.1 348.5c-6.1 0-11.1-5-11.1-11.1V72.2C0 66 5 61.1 11.1 61.1h284.5c6.1 0 11.1 5 11.1 11.1V257c0 6.1-5 11.1-11.1 11.1s-11.1-5-11.1-11.1V83.2H22.2v254.2c0 6.2-5 11.2-11.1 11.1 0 .1 0 .1 0 0z"
                    ></path>
                    <path
                      className="lineal-fill"
                      d="M48.8 398.5H258c20.8 0 37.7-16.9 37.7-37.7v-23.4H11.1v23.4c0 20.8 16.9 37.7 37.7 37.7z"
                    ></path>
                    <path
                      className="lineal-stroke"
                      d="M258 409.6H48.8C21.9 409.6 0 387.7 0 360.8v-23.4c0-6.1 5-11.1 11.1-11.1h284.5c6.1 0 11.1 5 11.1 11.1v23.4c0 26.9-21.8 48.7-48.7 48.8zM22.2 348.5v12.3c0 14.7 11.9 26.6 26.6 26.6H258c14.7 0 26.6-11.9 26.6-26.6v-12.3H22.2zM295.6 83.2H11.1C5 83.2 0 78.3 0 72.2V48.8C0 21.9 21.9 0 48.8 0H258c26.9 0 48.8 21.9 48.8 48.8v23.4c0 6.1-5 11-11.2 11zM22.2 61h262.3V48.8c0-14.7-11.9-26.5-26.5-26.6H48.8c-14.7 0-26.6 11.9-26.6 26.6V61z"
                    ></path>
                    <path
                      className="st2"
                      d="M65.1 130.8L85 145.3l23.3-28.9"
                    ></path>
                    <path
                      className="lineal-stroke"
                      d="M85 156.4c-2.3 0-4.6-.7-6.5-2.1l-20-14.4c-5-3.6-6.1-10.5-2.5-15.5s10.5-6.1 15.5-2.5l11.5 8.3 16.7-20.7c3.9-4.8 10.9-5.5 15.6-1.6s5.5 10.9 1.6 15.6l-23.3 28.8c-2 2.6-5.2 4.1-8.6 4.1zM241.7 142H159c-6.1-.2-11-5.3-10.8-11.4.2-5.9 4.9-10.6 10.8-10.8h82.7c6.1.2 11 5.3 10.8 11.4-.2 5.9-4.9 10.6-10.8 10.8z"
                    ></path>
                    <path
                      className="st2"
                      d="M65.1 204.8L85 219.2l23.3-28.8"
                    ></path>
                    <path
                      className="lineal-stroke"
                      d="M85 230.3c-2.3 0-4.6-.7-6.5-2.1l-20-14.4c-5-3.6-6.1-10.5-2.5-15.5s10.5-6.1 15.5-2.5l11.5 8.3 16.7-20.7c3.8-4.8 10.8-5.5 15.6-1.7s5.5 10.8 1.7 15.6l-23.3 28.9c-2.1 2.6-5.3 4.1-8.7 4.1zm156.7-14.4H159c-6.1 0-11.1-5-11.1-11.1s5-11.1 11.1-11.1h82.7c6.1 0 11.1 5 11.1 11.1s-5 11.1-11.1 11.1z"
                    ></path>
                    <path
                      className="st2"
                      d="M65.1 278.8L85 293.2l23.3-28.9"
                    ></path>
                    <path
                      className="lineal-stroke"
                      d="M85 304.3c-2.3 0-4.6-.7-6.5-2.1l-20-14.4c-5-3.6-6.1-10.5-2.5-15.5s10.5-6.1 15.5-2.5L83 278l16.7-20.7c3.9-4.8 10.9-5.5 15.6-1.7s5.5 10.9 1.7 15.6l-23.3 28.9c-2.1 2.7-5.3 4.2-8.7 4.2zm156.7-14.4H159c-6.1 0-11.1-5-11.1-11.1s5-11.1 11.1-11.1h82.7c6.1 0 11.1 5 11.1 11.1s-5 11.1-11.1 11.1z"
                    ></path>
                  </svg>
                  <h4 className="!mb-1">2. Auto-Generated Bio Link Page</h4>
                </div>

                <div className="xl:w-4/12 lg:w-4/12 md:w-4/12 w-full flex-[0_0_auto] xxl:!px-[20px] !px-[15px] max-w-full !mt-[30px]">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 294.2 409.6"
                    className="svg-inject icon-svg !w-[2.2rem] !h-[2.2rem]  !text-[#54a8c7] text-aqua !mb-3"
                  >
                    <path
                      className="lineal-stroke"
                      d="M31.2 342.9c-2.4 1.7-5.8 1.1-7.5-1.3-.6-.9-1-2-1-3.1v-77.1l68.9-77.2V150L5 247.1c-3.2 3.6-5 8.2-5 13v78.4c0 15.5 12.6 28.1 28.1 28.1 5.8 0 11.5-1.8 16.2-5.1l9.6-6.8-13.1-18.6-9.6 6.8zm258.1-95.8L200.7 148v34.2l70.8 79.3v77.1c0 3-2.4 5.4-5.4 5.4-1.1 0-2.2-.3-3.1-1l-9.6-6.7-13.1 18.6 9.6 6.7c12.7 8.9 30.3 5.9 39.2-6.9 3.3-4.7 5.1-10.4 5.1-16.1v-78.4c0-4.8-1.8-9.5-4.9-13.1zM147.1 409.6l-6.1-3.9c-2-1.3-49.3-32-49.3-72.5 0-40 23.3-72.6 53.1-74.2.8 0 1.6-.1 2.3-.1 4.1 0 8.1.6 12.1 1.7 9.1 2.7 17.6 8.4 24.6 16.6 11.9 14 18.7 34.4 18.7 55.9 0 40.5-47.2 71.2-49.3 72.5l-6.1 4zm0-127.9h-1c-17.5.9-31.7 24-31.7 51.5 0 19.6 19.9 39.1 32.6 48.9 12.7-9.8 32.6-29.3 32.6-48.9 0-16.2-4.9-31.2-13.3-41.1-4.1-4.8-8.8-8.1-13.6-9.5-1.8-.6-3.7-.9-5.6-.9z"
                    ></path>
                    <path
                      className="lineal-fill"
                      d="M147.1 11.4c-13.1 0-96.4 71.4-96.4 143.4 0 24.5 29.3 178.4 29.3 178.4h134.1s29.3-153.9 29.3-178.4c.1-72.4-83.9-143.4-96.3-143.4zm49.8 159.2c-1 4.9-2.7 9.6-5.1 14-.8 1.4-1.6 2.8-2.5 4.2-3.7 5.4-8.3 10.1-13.7 13.7-1.3.9-2.7 1.8-4.2 2.5-24.7 13.4-55.6 4.2-68.9-20.5s-4.2-55.6 20.5-68.9c7.4-4 15.8-6.2 24.3-6.1 3.4 0 6.9.3 10.2 1 4.9 1 9.6 2.7 14 5.1 7.2 3.9 13.4 9.5 17.9 16.3 5.7 8.4 8.7 18.3 8.7 28.4-.1 3.5-.4 6.9-1.2 10.3z"
                    ></path>
                    <path
                      className="lineal-stroke"
                      d="M147.1 222.6c-34.4 0-62.2-28-62.2-62.3 0-34.4 28-62.2 62.3-62.2 10.3 0 20.4 2.6 29.5 7.5 2.6 1.4 5.2 3.1 7.6 4.9 24.8 18.5 32.2 52.4 17.5 79.6-1 1.8-2 3.5-3.1 5.1-4.5 6.6-10.2 12.3-16.8 16.8-1.6 1.1-3.4 2.2-5.1 3.1-9.1 4.9-19.3 7.5-29.7 7.5zm0-101.7c-21.8-.3-39.7 17-40.1 38.8s17 39.7 38.8 40.1h1.3c2.7 0 5.4-.3 8-.8 3.8-.8 7.4-2.1 10.8-4 1.1-.6 2.2-1.3 3.2-2 4.2-2.8 7.8-6.5 10.7-10.7.7-1.1 1.4-2.1 2-3.3 1.9-3.4 3.2-7 4-10.8.5-2.6.8-5.3.8-8 0-12.5-5.9-24.2-15.9-31.6-1.5-1.1-3.1-2.2-4.8-3.1-3.4-1.9-7-3.2-10.8-4-2.6-.5-5.3-.8-8-.8v.2z"
                    ></path>
                    <path
                      className="lineal-stroke"
                      d="M89.5 321.8h115.2c9-47.9 27.4-149.1 27.4-167 0-63-69.5-123.9-85-131.5-6.7 3.3-26.8 18.4-46.8 43.2-24.6 30.4-38.2 61.8-38.2 88.4 0 17.9 18.4 119.1 27.4 166.9zm134.1 22.8H70.7l-1.8-9.3c-3-15.9-29.5-155.7-29.5-180.6 0-42.2 25-80.9 45.9-106C101.1 29.9 131.8 0 147.1 0c15 0 45.7 29.8 61.5 48.6 21.1 25.1 46.2 63.8 46.2 106.2 0 24.9-26.5 164.7-29.5 180.6l-1.7 9.2z"
                    ></path>
                  </svg>
                  <h4 className="!mb-1">3. QR Code Linked to Welcome Flow</h4>
                </div>
              </div>
            </div>
          </div>
          <div className="flex flex-wrap mx-[-15px] xl:mx-[-25px] lg:mx-[-20px] items-center">
            <div className="xl:w-6/12 lg:w-6/12 w-full flex-[0_0_auto] xl:!px-[25px] lg:!px-[20px] !px-[15px] max-w-full">
              <h3 className="!text-[calc(1.315rem_+_0.78vw)] font-bold xl:!text-[1.8rem] !leading-[1.25] !mb-5">
                Drag-and-drop chatbot builder
              </h3>
              <p className="!mb-8">
                Create triggers, set up automated replies, and develop your
                chatbot architecture to build an effective sales pipeline. Plan
                your message flow and add helpful information or instructions to
                ensure your subscribers can easily find answers to their
                questions or place their orders.Create triggers, set up
                automated replies, and develop your chatbot architecture to
                build an effective sales pipeline. Plan your message flow and
                add helpful information or instructions to ensure your
                subscribers can easily find answers to their questions or place
                their orders.
              </p>
            </div>
            <div className="xl:w-6/12 lg:w-6/12 w-full flex-[0_0_auto] xl:!px-[25px] lg:!px-[20px] !px-[15px] max-w-full">
              <figure className="m-0 p-0">
                <img
                  className="w-full max-w-full !h-auto"
                  src="https://sandbox-tailwind-template.netlify.app/assets/img/photos/device.png"
                  srcSet="https://sandbox-tailwind-template.netlify.app/assets/img/photos/device.png"
                  alt="image"
                />
              </figure>
            </div>
          </div>
          <div className="flex flex-wrap mx-[-15px] xl:mx-[-25px] lg:mx-[-20px] items-center">
            <div className="xl:w-6/12 lg:w-6/12 w-full flex-[0_0_auto] xl:!px-[25px] lg:!px-[20px] !px-[15px] max-w-full">
              <figure className="m-0 p-0">
                <img
                  className="w-full max-w-full !h-auto"
                  src="https://www.spcdn.org/images/whatsapp/whatsapp-en/pic_short-meaningful-dialog01-en.png"
                  srcSet="https://www.spcdn.org/images/whatsapp/whatsapp-en/pic_short-meaningful-dialog01-en.png"
                  alt="image"
                />
              </figure>
            </div>

            <div className="xl:w-6/12 lg:w-6/12 w-full flex-[0_0_auto] xl:!px-[25px] lg:!px-[20px] !px-[15px] max-w-full">
              <h3 className="!text-[calc(1.315rem_+_0.78vw)] font-bold xl:!text-[1.8rem] !leading-[1.25] !mb-5">
                Conversations with chatbot subscribers and teamwork
              </h3>
              <p className="!mb-8">
                Invite your colleagues to share your SendPulse account to boost
                collaboration and streamline customer support. Assign team
                members to manage your new chatbots, track marketing
                performance, and increase daily workflow efficiency. Chatbot
                assignees can monitor conversation history and subscriber data,
                reply to specific messages, send instant reactions, and edit or
                delete messages. Beyond that, assigned team members can add flow
                comments that are not visible to chatbot subscribers, manage
                auto-replies, and create CRM deals — all without leaving the
                chat.
              </p>
            </div>
          </div>
        </div>
      </section>
      <section
        className="wrapper image-wrapper bg-full bg-image bg-overlay bg-overlay-light-600 [background-size:100%] bg-[center_center] bg-no-repeat bg-scroll relative z-0 before:content-[''] before:block before:absolute before:z-[1] before:w-full before:h-full before:left-0 before:top-0 before:bg-[rgba(255,255,255,.6)]"
        style={{
          backgroundImage:
            "url(https://sandbox-tailwind-template.netlify.app/assets/img/photos/bg23.png)",
        }}
      >
        <div className="overflow-hidden" style={{ zIndex: "1" }}>
          <div className="divider divider-alt !text-[#fefefe] mx-[-0.5rem]">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 100">
              <g fill="currentColor">
                <polygon points="1440 100 0 15 0 0 1440 0 1440 100"></polygon>
              </g>
            </svg>
          </div>
        </div>

        <div className="container pt-10 xl:pt-10 lg:pt-10 md:pt-16 pb-8 xl:pb-10 lg:pb-10 md:pb-10 text-center xl:text-left lg:text-left">
          <div className="flex flex-wrap mx-0 !mt-[-50px]">
            <div className="md:w-10/12 md:!ml-[8.33333333%] lg:!ml-0 lg:w-6/12 xl:!ml-0 xl:w-5/12 w-full flex-[0_0_auto] max-w-full !relative !mt-[50px]">
              <img
                src="https://sandbox-tailwind-template.netlify.app//assets/img/svg/doodle2.svg"
                className="!h-[5rem] !absolute hidden xl:block lg:block"
                style={{ bottom: "36%", right: "-25%" }}
                alt="image"
              />
              <img
                src="https://sandbox-tailwind-template.netlify.app//assets/img/svg/doodle6.svg"
                className="!h-[4.5rem] !absolute hidden xl:block lg:block"
                style={{ top: "-20%", left: "-25%" }}
                alt="image"
              />
              <h2 className="!text-[calc(1.325rem_+_0.9vw)] font-semibold !leading-[1.2] xl:!text-[2rem] !mb-4">
                Manage chatbots on your
                <span className="text-gradient gradient-7">smartphone</span>
              </h2>
              <p className="lead !text-[1.2rem] !leading-[1.5] !font-normal !mb-7">
                Install the app to quickly respond to WhatsApp, Facebook,
                Instagram, and other messages, process subscriber data, analyze
                chatbot statistics, and keep up with the latest chatbot builder
                updates.
              </p>
            </div>

            <div className="xl:w-6/12 lg:w-6/12 w-full flex-[0_0_auto] max-w-full !ml-auto !mt-lg-n15 xl:!mt-[-10rem] lg:!mt-[-5rem] !mt-[50px]">
              <figure className="m-0 p-0">
                <img
                  className="w-full max-w-full !h-auto"
                  src="https://sandbox-tailwind-template.netlify.app//assets/img/photos/devices6.png"
                  srcSet="https://sandbox-tailwind-template.netlify.app//assets/img/photos/devices6.png"
                  alt="image"
                />
              </figure>
            </div>
          </div>
        </div>
      </section>
      <PhaseOfdevelopments />
      <Faq />
    </>
  );
}

export default Whatsappchatbot;
